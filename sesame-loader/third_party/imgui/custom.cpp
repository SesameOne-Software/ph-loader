...
ImVec2 g_window_dim = ImVec2( 300 , 216 );
...
void ImGui::custom::AddTab( const char* icon ) {
    ...
    const auto menu_bar_width = window_size.x * ( 1.0f / 8.0f );
    ...
    const auto tab_id = GetID( std::string( icon ).append( _("##tab_icon") ).c_str( ) );
...
void ImGui::custom::EndTabs( ) {
    ...
    ImGui::BeginChildFrame( GetID( _("##main_space") ), ImVec2( window_size.x - menu_bar_width, window_size.y - menu_bar_width - title_size.y - style.FramePadding.y * 2.0f ), ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoMove );
...
//
    ...
    const auto node_name = std::string( title ).append( _("##node") ).append( std::to_string( *g_cur_tab_ptr ) );
...
bool ImGui::custom::Begin( const char* name, bool* p_open, ImFont* small_font ) {
    ...
    SetNextWindowSize( ImVec2( g_window_dim.x, g_window_dim.y ) );
    SetNextWindowPos( ImVec2( 0.0f, 0.0f ) );

    if ( !ImGui::Begin(
        name,
        p_open,
        ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoMove ) )
        return false;
...
bool ImGui::custom::Spinner( const char* label , float radius , int thickness , const ImU32& color ) {
    ImGuiWindow* window = GetCurrentWindow( );
    if ( window->SkipItems )
        return false;
    ...
    for ( int i = 0; i < num_segments; i++ ) {
        ...
        window->DrawList->PathLineTo( ImVec2( centre.x + ImCos( a + g.Time * 8.0f ) * radius ,
            centre.y + ImSin( a + g.Time * 8.0f ) * radius ) );
    }
